...
```{r}
data <- read.table("data/data_TOT.txt", header = TRUE)[, -4]
```


Les unités de temps sont dans le vecteur `Y`.
```{r}
Y <- data$Y + 1825 # Première année : 1826
nTemps <- length(Y)
```
...
- $T$ : le nombre de taxonomistes actifs chaque année.
```{r}
# 5 au début, 20 à la fin, bruité       # A remplacer par les vraies données
T <- 10^(1 + Y/nTemps + rnorm(length(Y), sd=0.02)) /5
```
...
iterations <- 1e+6
# Ecart-type de la marche aléatoire (pour chaque paramètre)
sigma_prop <- c(2E-3, 2E-5, 1E5, 2E-1)
# Valeur initiale des paramètres
theta_0 <- c(
  beta_1/2,   # beta_1
  beta_2/2,   # beta_2
