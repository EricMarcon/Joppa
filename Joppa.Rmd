...
Les unités de temps sont dans le vecteur `Y`.
```{r}
Y <- data$Y # Première année : 1891, chaque période dure 5 ans
nTemps <- length(Y)
```
...
```{r}
S <- data$S
S_0 <- 1 + 35 # L. terrestris par Linné 
```
...
```{r}
acf(chain[-(1:burn_in), 1:(ncol(chain)-2)], lag.max = 10000)
```
...
# Figure
tibble(Y, Estime=sum_S[-1], Observe=cumsum(data$S) + 35) %>% 
  pivot_longer(cols = c("Estime", "Observe")) %>% 
  ggplot() +
    geom_line(aes(x=Y, y=value, color=name)) +
    scale_y_log10()
```
...
Estimation : 12000 espèces, mode à gauche.
